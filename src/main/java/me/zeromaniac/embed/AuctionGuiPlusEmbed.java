...
public class AuctionGuiPlusEmbed extends AbstractEmbed {

    public AuctionGuiPlusEmbed(OfflinePlayer player, OfflinePlayer bidder, OfflinePlayer buyer, ItemStack stack,
            int id, String duration, double price, double startingPrice, double increment, double amount,
    ...
    protected void initDebug() {
        debug = ConfigHandler.getMainConfig().getConfig()
                .getBoolean(MainConfigDefaults.IS_AUCTION_GUI_PLUS_DEBUG.getPath());
    }

    @Override
    protected boolean isEnabled(String type) {
        enabled = config.getBoolean(type + ".Embed.Enabled")
                && ConfigHandler.getMainConfig().getIsAuctionGUIPlusEnabled();
